// Lenis بإعدادات مخصصة لأعلى سلاسة ممكنة 120Hz FPS
const lenis = new Lenis({
    duration: 1.2,
    easing: (t) => Math.min(1, 1.001 - Math.pow(2, -10 * t)),
    direction: 'vertical',
    smooth: true,
    lerp: 0.08, // تم التعديل لتتبع أسرع وأكثر نعومة مع الشاشات السريعة
    smoothWheel: true,
    mouseMultiplier: 1,
    touchMultiplier: 2,
});
...
if (window.matchMedia("(pointer: fine)").matches) {
    ...
    tiltElements.forEach(el => {
        el.addEventListener('mousemove', (e) => {
            ...
            el.style.transform = `translate3d(0, -10px, 0) rotateY(${xPos}deg) rotateX(${yPos}deg)`;
        });
...
window.addEventListener('scroll', () => {
    if(window.scrollY < window.innerHeight) {
        heroImg.style.transform = `translate3d(0, ${window.scrollY * 0.15}px, 0)`;
    }
});
...
lenis.on('scroll', (e) => {
    let scrollTotal = document.documentElement.scrollTop || document.body.scrollTop;
